...
> - 하나의 HTML Element에 여러가지 이름을 사용할 수 있음
> - 스타일을 공통 적용, 효과를 여러 곳에 동일하게 적용할 때 사용

```
<h1 class="heading">제목</h1>
<h2 class="heading">제목</h2> (O)
...
>
> - name(tag), id, class 3가지를 사용해서 선택자를 표현
>
...
>
> - 감산 혼합 : 잉크 혼합
>   - 혼합하는 색이 많을 수록 어두워짐, 모든 색을 혼합 => 검정색
>   - 프린트 : 잉크 색을 이용
>   - 컬러모드 : C(yan)M(agenta)Y(ellow)(blac)K

> RGB 모드
>
> 컴퓨터 사용하는 숫자 진법 : 2진법(0,1)
>
> 데이터 저장 최소 단위 : bit
>
> - 저장 공간 1칸 : 0,1중에 하나를 저장
>
...
=> font 색상 적용은 상속이 되어 child 요소에도 글꼴 색이 빨간색으로 적용
```
...
=> 맑은 고딕 폰트를 찾고 있으면 적용, 없으면 돋움 폰트를 찾고 있으면 적용, 없으면 sans-serif(브라우저 기본 폰트)를 적용
```
